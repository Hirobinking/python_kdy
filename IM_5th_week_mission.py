...
'''
class Monster:                                        ## [문법] class 클래스이름:
    def say(self):
        print('나는 아주아주 잘생긴 몬스터다!')            ##           def __init__(self, 입력변수들):
goblin = Monster()                                     ##초기화 실행 문장
goblin.say()
'''
## 생성자 연습문제
...
class Monster:
    def __init__(self, name, health, attack, speed):
        ...
        self.speed = speed
    def say(self):
        print(f"나는 아주아주 무서운 {self.name}(이)다!!!")

goblin = Monster('고블린', 100, 30, 30)
wolf = Monster('늑대', 300, 150, 50)
goblin.say()
wolf.say()
'''
