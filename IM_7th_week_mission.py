...
'''
num1 = float(input('첫번째 숫자를 입력해 주세용'))
num2 = float(input('두번째 숫자를 입력해 주세용'))
print(num1+num2)
'''
...
'''
try:
    won = float(input('원화 금액을 입력하여주세용'))
    dollar = float(input('달러 금액을 입력하여주세용'))
    exchange_rate = won / dollar
    print(f"환율:{exchange_rate}")
except ValueError as e:
    print('문자열 처리 예외가 발생하였습니다.', e)
except ZeroDivisionError as d:
    print('달러에는 0이 들어갈 수 없습니다.', d)
else:
...
'''
words = ['oh sujin', 'Pneumonoultramicroscopicsilicovolcanoconiosis', 'Supercalifragilisticexpialidocious']
i=0
count = 0
while True:
        print(words[i])
