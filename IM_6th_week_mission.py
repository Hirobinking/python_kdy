...
class Dragon(Monster):
    def move(self):
        print(f"[{self.name}]비행하기!!!")

A_shark =  Shark("야생의 상어", 1000, 2000)
A_shark.move()

B_dragon = Dragon("야생의 드래곤", 2000000000, 10000000000000000)
B_dragon.move()

C_wolf = Wolf("야생의 늑대", 1000, 2000)
C_wolf.move()
'''
...
class Dragon(Monster):
    ...
    def skill(self):
        print(f"[{self.skills[random.randint(0, 2)]}]이 스킬을 시전했다! 효과는 굉장했다!")

    def move(self):
        print(f"[{self.name}]비행하기!!!")

D = Dragon('김도연', 400, 5000)
D.skill()

wolf = Wolf("늑대", 200, 300)
print(f"늑대 생성 후: {wolf.max_num}")

baby_shark = Shark("아기상어", 200, 3000)
print(f"상어 생성 후: {baby_shark.max_num}")

D = Dragon('김도연', 400, 5000)
print(f"드레곤 생성 후: {D.max_num}")
